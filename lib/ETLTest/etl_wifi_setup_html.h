...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .password-section { background: #F2F2F7; border-radius: 10px; padding: 16px; margin-bottom: 20px; }
        ...
        body.large-font .selected-network-title { font-size: 20px; }
        .selected-network-divider { height: 1px; background: #C6C6C8; margin-bottom: 16px; }
        .connection-error { background: #FF3B30; border-radius: 8px; padding: 12px 16px; margin-bottom: 12px; display: flex; align-items: center; gap: 12px; }
        .connection-error-icon { font-size: 20px; flex-shrink: 0; }
        .connection-error-text { flex: 1; font-size: 14px; color: #FFFFFF; font-weight: 500; }
        body.large-font .connection-error-text { font-size: 16px; }
        .retry-btn { padding: 6px 12px; border: 1px solid #FFFFFF; border-radius: 6px; background: transparent; color: #FFFFFF; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
        body.large-font .retry-btn { font-size: 15px; padding: 8px 14px; }
...
<body>
    <div class="container">
        ...
        <div class="password-section hidden" id="passwordSection">
            ...
            <div class="connection-error hidden" id="connectionError">
                <span class="connection-error-icon">⚠️</span>
                <span class="connection-error-text" data-i18n="connection_failed">Connection failed. Check password.</span>
                <button class="retry-btn" id="retryBtn" data-i18n="retry_btn">Retry</button>
            </div>
    ...
    <script>
        ...
        function showPasswordSection(ssid, isDisconnect = false) { passwordSection.classList.remove('hidden'); selectedNetworkTitle.textContent = ssid; hideConnectionError(); if (isDisconnect) { passwordInputGroup.classList.add('hidden'); connectBtn.textContent = translations[currentLang].disconnect_btn; connectBtn.className = 'btn btn-secondary'; } else { passwordInputGroup.classList.remove('hidden'); connectBtn.textContent = translations[currentLang].join_btn; connectBtn.className = 'btn btn-primary'; } }
        ...
        refreshBtn.addEventListener('click', scanNetworks); connectBtn.addEventListener('click', connectToNetwork); retryBtn.addEventListener('click', retryConnection); fontToggleBtn.addEventListener('click', toggleLargeFont); showPasswordBtn.addEventListener('click', () => togglePassword(passwordInput, showPasswordBtn)); showApPasswordBtn.addEventListener('click', () => togglePassword(apPasswordInput, showApPasswordBtn)); saveRebootBtn.addEventListener('click', saveAndReboot); factoryResetBtn.addEventListener('click', factoryReset); applyApSettingsBtn.addEventListener('click', applyApSettings); document.querySelectorAll('.lang-btn').forEach(btn => { btn.addEventListener('click', () => setLanguage(btn.dataset.lang)); }); document.addEventListener('click', (e) => { if (!passwordSection.contains(e.target) && !e.target.closest('.network-item')) { if (!isConnected) passwordSection.classList.add('hidden'); } });
        init();
    </script>
</body>
</html>
