...
namespace light_mqtt
{
    ...
    bool light_manager::begin(etl::shared_ptr<etl::wifi::manager> wifi_mgr)
    {
        ...
        auto status_cb = [this](etl::mqtt::status_t status) {
    ...
    void light_manager::on_mqtt_status_changed(etl::mqtt::status_t status)
    {
        switch (status)
        {
        case etl::mqtt::status_t::connected:
            Serial.println(F("[light_mqtt] Connected to MQTT broker"));
            // Публикация начального состояния при подключении
            publish_current_state();
            break;

        case etl::mqtt::status_t::disconnected:
            Serial.println(F("[light_mqtt] Disconnected from MQTT broker"));
            break;

        case etl::mqtt::status_t::error:
    ...
    etl::shared_ptr<light_mqtt::light_manager> g_light_mqtt_mgr;
