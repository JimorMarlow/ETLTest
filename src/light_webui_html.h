...
let sendTimeout=null;function sendState(){if(sendTimeout)return;sendTimeout=setTimeout(()=>{sendTimeout=null},100);const data={power:deviceState.power,brightness:deviceState.brightness};fetch('/api/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).catch(e=>console.log('Send error:',e))}
