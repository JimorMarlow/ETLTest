...
.connection-icon.mqtt{display:none;width:20px;height:20px;background:#007AFF;border-radius:50%;color:#FFFFFF;align-items:center;justify-content:center;font-size:14px}
...
function sendState(){ if(Date.now()-lastExternalUpdate<EXTERNAL_LOCK_MS){console.log('[UI] Suppressing send: external update lock active');return;} const data={power:deviceState.power,brightness:deviceState.brightness};fetch('/api/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).catch(e=>console.log('Send error:',e))}
